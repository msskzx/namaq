...
- A battle record carries four counts: `muslimForceCount`, `nonMuslimForceCount`, `muslimDeathCount` and `nonMuslimDeathCount`. They are separate columns because the sources count the two sides separately, and each is cited like any other value, so a battle whose numbers nobody states carries none. Where readings compete, the module takes one and the others stay as `DISPUTED` claims in the batch rather than being averaged or dropped: Badr is 313 against 950 with 14 and 70 dead, and Uhud is 700 against 3000 with 70 dead, which is the count chapter five had to leave on the page for want of a column.
