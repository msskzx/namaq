...
- `/graphs` has a single search box with no mode dropdown: picking a person adds them as an additional `subject` root in the current exploration and selects them (existing subjects, expansions, and filters are preserved, nothing is expanded implicitly), while picking a non-person result just selects/centers it without adding a root. It merges two sources into one suggestion list: `GET /api/people/suggest` for people, and a client-side match against whatever titles/battles/events the graph has already loaded (that endpoint only ever covers people). Ancestors/Paternal lineage/Descendants are exploration actions on the selected subject (see the selected-subject panel), not search modes.
- `/api/people/suggest` finds a person whether or not they have a PostgreSQL profile row: it queries PostgreSQL and Neo4j and ranks both into one list (match quality, then nasabRank), rather than PostgreSQL results always outranking a graph-only person by construction. A graph-only person (present in Neo4j, e.g. a deep lineage-only ancestor, no PostgreSQL row — see `CONTEXT.md`) has no profile page, so their suggestion and their selected-subject panel both omit the "View profile" link rather than linking to a page that doesn't exist.
...
- `npm run graph:layout` computes a cross-type prominence rank (`graphRank`), a Louvain community (`clusterId`), and a precomputed 2D layout position (`layoutX`/`layoutY`) over the *entire* unified Person+Battle+Title+Event graph — every subject, not just ones with a PostgreSQL row. Rank/cluster/layout are stored on each entity's Postgres row as before; `layoutX`/`layoutY` are additionally written to every matching Neo4j node (including PostgreSQL-less, graph-only subjects, e.g. deep lineage-only ancestors), which is what `/api/graph` actually reads and pins every node to — no live force layout runs in the browser at all. Zoom-based level-of-detail (thinning low-rank nodes as you zoom out) is a further follow-up.
...
`GET /api/graph` is the single graph endpoint behind `/graphs` — there is no separate `/api/graph/all`, `/api/graph/battles`, or `/api/graph/titles` to keep in sync with it. With no query parameters it runs one unified Neo4j query across every Person/Battle/Title/Event node and relationship and joins in each node's `graphRank`/`clusterId`/`layoutX`/`layoutY` (and, for people, `nasabRank`) from PostgreSQL. From there:
...
The person half of `/graphs`' search comes from `GET /api/people/suggest` (PostgreSQL-backed, person-only). Its non-person half needs no endpoint of its own — it matches against whatever `/api/graph` already returned for the current `kind` filter.
