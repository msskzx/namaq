...
- `/graphs` has a single search box with no mode dropdown: picking a person adds them as an additional `subject` root in the current exploration and selects them (existing subjects, expansions, and filters are preserved, nothing is expanded implicitly), while picking a non-person result just selects/centers it without adding a root. It merges two sources into one suggestion list: Postgres-backed autocomplete for people, and a client-side match against whatever titles/battles/events the graph has already loaded (which the Postgres-backed side doesn't cover). Ancestors/Paternal lineage/Descendants are exploration actions on the selected subject (see the selected-subject panel), not search modes.
