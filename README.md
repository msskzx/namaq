...
- The selected-subject panel offers four controls: **All direct relations**, which expands the selection along every active filter that has neighbors, plus Ancestors, Paternal lineage and Descendants. There is no per-relation expansion button — relation-by-relation control lives in the Filters panel, so "all" never fetches a type that is switched off. Global filters introduce capped one-hop neighbors and report count changes. **Show full graph** (`full=1`) adds the enabled dataset to the current exploration; **Start over** returns to a fresh visit — Muhammad as the only root, default node kinds, and the default filters back on. Both preserve language/theme and support browser Back/Forward.
- An absent `filter` parameter enables family and title relations by default; companionship, battle participation, and event relations are opt-in. `filter=` explicitly disables all global filters, including after refresh, and is what the All relations switch writes when you turn everything off. **Start over** clears the parameter instead, so the defaults come back and the Filters panel matches what the graph shows.
