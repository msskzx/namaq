...
- The embedded (non-workspace) graph canvas has a fullscreen mode. Fullscreen brings the workspace's selected-subject controls along with the relation/kind toggles, in a floating panel over the graph, plus a close button. Global search stays behind, since it navigates the page URL while an embedded exploration is kept in the component.
- Selecting a node takes the learner to `/people/[slug]`.
- Person pages embed the same graph engine the workspace uses, opened on that person under the shared rule above: every direct relation they have, plus their ancestors and descendants. The page shows the canvas alone; the filters, the selected-subject actions and the node list all live in fullscreen, reached by the one button on the canvas. Its exploration is kept in the component rather than the address bar, so a profile link carries the person and nothing else.
