...
- Abu Ubaydah ibn al-Jarrah and Talhah ibn Ubaydullah are authored entirely in the catalog: their fields, title assignments, battle participations and their cited relation to their father come from `data/catalog/`, written by `npm run catalog:project` and `npm run catalog:project-graph`, and their old seed entries are gone but for the Qur'an links the catalog has no shape for. Every other person still comes from the seed files, so both paths are live at once.
