...
- Global filters start off. A fresh visit and **Start over** open on the Prophet with his ancestors and descendants as his own expansions, so nothing global has to be on to see a family. A person's profile graph opens on that person under the same rule. Neither line is capped. Bulk and group switches leave companionship in whatever state its own switch set, in both directions. Battle participation gains status switches (martyred, died, injured, captured, was captured, absent, and status not recorded) that match with OR; a participation that does not match disappears while the person stays if another relation supports them.
