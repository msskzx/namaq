...
- The search bar on `/graphs` provides Postgres-backed autocomplete and keeps the selected view in the URL. It has three modes: **Relations** (a person's direct neighbourhood), **Ancestors** (paternal/maternal lineage upward via `ancestorsOf`), and **Descendants** (children, grandchildren, etc. downward via `descendantsOf`) — the same `SON`/`DAUGHTER` chain, walked in the opposite direction. Alongside it, a second, client-side "jump to node" search box matches any already-loaded node by name/slug (Arabic-normalization aware) and selects/centers it — useful for titles, battles, and events, which the Postgres-backed search doesn't cover. Both search boxes and every filter are always present together; nothing is hidden based on which kinds are currently shown.
...
Graph-search suggestions (the Relations/Ancestors/Descendants person lookup) come from `GET /api/people/suggest` (PostgreSQL-backed, person-only). The second, generic "jump to node" search on `/graphs` needs no endpoint of its own — it matches against whatever `/api/graph` already returned for the current `kind` filter.
