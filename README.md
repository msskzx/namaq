...
- Source material has a shelf of its own at `/sources`: every work the evidence is read from, with its author, editor and edition, and how many entries and printed pages of it have been read. Opening one opens the book at its contents -- the entries read from it, each with its length -- and choosing an entry opens the same reader the profile uses, paging through the work's entries rather than a person's books — a source is one edition of one work, so two editions of a book stand as two shelf entries. The reader is one component either way (`src/components/people/SourceAccountReader.tsx`); only the accounts it may page through differ, which is why a profile showing a subject read from several works gets the book selector for free.
