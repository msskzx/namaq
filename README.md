...
- An absent `filter` parameter enables family and title relations by default; companionship, battle participation, and event relations are opt-in. `filter=` explicitly disables all global filters, including after refresh. **Start over** uses that empty set to restore Muhammad alone.
