...
- Relationship controls carry an explicit scope: **Selected subject** (the default) expands that one subject, **Entire exploration** filters the whole graph. With nothing selected the local controls are disabled rather than silently acting globally. The selected-subject panel adds **All direct relations**, which expands every recorded relation of the selection, plus Ancestors, Paternal lineage, Descendants, **Collapse branch**, **Remove from exploration**, and **Keep only this subject**. **Show full graph** (`full=1`) adds the enabled dataset, respecting explicit removals; **Start over** returns to a fresh visit. All of them support browser Back/Forward.
- Global filters start off. A fresh visit and **Start over** open on the Prophet with his recorded wives, sons, daughters, and grandchildren as his own expansions, so nothing global has to be on to see a family. Bulk and group switches leave companionship in whatever state its own switch set, in both directions. Battle participation gains status switches (martyred, died, injured, captured, was captured, absent, and status not recorded) that match with OR; a participation that does not match disappears while the person stays if another relation supports them.
