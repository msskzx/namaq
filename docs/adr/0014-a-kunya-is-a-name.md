...
place. It is no longer a title, and the value moves to his kunya in the seed
entry that still authors him: no batch has read his entry, and the catalog is
for what a source was read for, not a place to park a value. The title row
survives in the stores until something removes it, which `npm run
titles:validate` reports meanwhile.
