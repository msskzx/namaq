...
export default function SourceAccountReader({ slug }: SourceAccountReaderProps) {
  ...
  // Fetched separately from the page: the index covers every page of the
  ...
  // The section open on screen: the last heading at or before this page.
  // Sequence alone can't key the option (more than one heading can share a
  // page, per pageHeadings in src/lib/history/sectionHeadings.ts), so this
  // tracks the heading's position in the list instead.
  let currentSectionIndex = -1;
  sections.forEach((candidate, index) => {
    if (candidate.sequence <= current.sequence) currentSectionIndex = index;
  });
      ...
      <div className={accounts.length > 1 || sections.length > 0 ? 'flex flex-wrap items-center gap-3 mb-4' : 'hidden'}>
        ...
        {sections.length > 0 && (
          <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            ...
            <select
              className="rounded border border-amber-400 bg-white dark:bg-gray-950 px-2 py-1 text-sm text-gray-800 dark:text-gray-100 max-w-[16rem]"
              dir="rtl"
              value={currentSectionIndex}
              onChange={(changed) => {
                const target = sections[Number(changed.target.value)];
                if (target) setSelection(account.id, target.sequence);
              }}
            >
              {currentSectionIndex === -1 && (
                <option value={-1} disabled>
                  {language === 'ar' ? 'اختر قسما للانتقال إليه' : 'Jump to a section'}
                </option>
              )}
